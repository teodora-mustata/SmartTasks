...
namespace SmartTasksAPI.Tests.Services
{
    ...
    public class UserServiceTests
    {
        ...
        [Fact]
        public async Task CreateAsync_ShouldNormalizeEmail_ToLowercase()
        {
            var userRepository = new Mock<IUserRepository>();
            User? addedUser = null;

            userRepository.Setup(x => x.GetByEmailAsync("TEST@EXAMPLE.COM"))
                .ReturnsAsync((User?)null);
            userRepository.Setup(x => x.AddAsync(It.IsAny<User>()))
                .Callback<User>(user => addedUser = user)
                .ReturnsAsync((User u) => u);
            ...
            await service.CreateAsync("Test User", "TEST@EXAMPLE.COM");

            Assert.Equal("test@example.com", addedUser?.Email);
            userRepository.Verify(x => x.GetByEmailAsync("TEST@EXAMPLE.COM"), Times.Once);
        }

        [Fact]
        public async Task CreateAsync_ShouldTrimWhitespace_FromFullNameAndEmail()
        {
            var userRepository = new Mock<IUserRepository>();
            User? addedUser = null;

            userRepository.Setup(x => x.GetByEmailAsync("user@example.com"))
                .ReturnsAsync((User?)null);
            userRepository.Setup(x => x.AddAsync(It.IsAny<User>()))
                .Callback<User>(user => addedUser = user)
                .ReturnsAsync((User u) => u);
        ...
        [Fact]
        public async Task GetByIdAsync_ShouldReturnUserFromRepository_WhenUserExists()
        {
            var userId = Guid.NewGuid();
            var expected = new User { Id = userId, FullName = "Jane", Email = "jane@test.com" };
            var userRepository = new Mock<IUserRepository>();
            ...
            Assert.Same(expected, result);
            userRepository.Verify(x => x.GetByIdAsync(userId), Times.Once);
        }
